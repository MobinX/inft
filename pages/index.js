...
const Main = ({children, className}) => {
  ...
  <div className="relative min-h-screen min-w-full  bg-gray-800  ">
    ...
    <main className={`absolute inset-0 min-h-screen min-w-full bg-white-1 blur-xl  ${className}`}>
      {children}
    </main>
  </div>)
  }
...
export default function Home() {
  ...
  return (
    <Main className="col min-h-screen min-w-full p-0">
      <div className="col pt-4 px-7 md:pt-7 md:px-9  pb-14">
        {/* navbar */}
        <div className="row justify-between text-white">
          {/* icon container */}
          <div className="row space-x-2">
            <div className="rounded-full bg-purple-600 w-3 p-3">
              {" "}
              {/*icon */}
            </div>
        ...
        <div className="row space-x-0 space-y-6 md:space-x-24 flex-wrap md:flex-nowrap px-4 py-16">
          ...
          <div className="col justify-start items-start w-full md:w-1/2 space-y-3 md:space-y-6 text-white">
            ...
            <Animate.p className="md:text-lg ">
              ...
              create a digital wallet where {"you'll"} securely store yuor
              crypto currency that is used to buy, sell and create NFTs.{" "}
            </Animate.p>
            {/* btn */}
